#lang racket/base
;;AML Note that 'expr should be read as (QUOTE expr) and not (quote expr)

;; Readtable-based AutoLisp reading
...
;;       #f
;;       (list 'QUOTE
;;             (read-syntax/recursive src port #f (current-readtable)))
                           ...
                           (cond
                            [(memq char '(#\a #\b #\t #\n #\v #\f #\r #\\ #\"))
                             (list* (subbytes bytes bpos (caar m))
                                    ...
                                    (loop (cdar m)))]
                            [(char<=? #\0 char #\9)
                             ;;extract next two chars
                             (let* ([octal-bytes (subbytes bytes (add1 (caar m)) (+ (add1 (caar m)) 3))]
                                    [str (bytes->string/utf-8 octal-bytes)]
                                    [num (string->number str 8)])
                               (list* (list->bytes (list num)) ;;the bytes procedure is shadowed.
                                      ;;This is one point for CL. 
                                      #""
                                      (loop (+ (add1 (caar m)) 3))))]
                            [(eq? char #\x)
...
(define (make-symbol-or-nil name)
  (if (nil-name? name)
      (nil)
...
(define read-list
  (case-lambda
    [(ch port)
     ;; `read' mode
     (let ([lst (read/recursive port ch #f)])
       (if (null? lst)
           (nil)
           lst))]
    [(ch port src line col pos)
     ;; `read-syntax' mode
     (let ([lst-stx (read-syntax/recursive src port ch #f)])
       (if (null? (syntax->datum lst-stx))
           (datum->syntax
            #f
            (nil)
            (let-values ([(l c p) (port-next-location port)])
...
(define autolisp-readtable
  (make-readtable 
   #f
   #\( 'terminating-macro read-list
;;   #\' 'terminating-macro read-quoted
...
(define (with-autolisp-reader-parameters thunk)
  (parameterize ([current-readtable autolisp-readtable]
